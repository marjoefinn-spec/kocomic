...
| 评论 | 看不到 | 每话末尾附上本话吐槽 |
| 域名轮换 | 只能自己改配置 | 一键自动找回,还会从接口和报错里捡线索 |
...
拷贝漫画的接口域名是会轮换的 —— 现在这个 `api.copy202601.com`,名字里就带着年月。
插件里做了一键找回:**设置 → 接口域名 → 自动查找可用域名**,
或者遇到「连不上接口」时会直接弹出来问你要不要找。

它按这个顺序挨个探活(命中就停,过程中点屏幕可以中止):

1. 现在用的这个(先确认是不是真挂了)
2. **以前用通过的**(记在设置里,菜单里也能一键切回)
3. 从接口自己吐出来的线索:
   - `/api/v3/system/network2` 里的官方站点域名
   - **报错文案里的网址** —— 比如 210 那条会说「請到官網更新最新APP(https://www.copy3000.com/download)」,
     实测 `api.copy3000.com` 就是活的,插件会自动把它捡下来当候选
4. 一串常驻老域名:api.mangacopy.com、api.copymanga.fun、api.copy3000.com…
5. **按规律现编**:`api.copy{年月}.com`,从三个月后往前倒推两年多

实测同时活着的就有好几个(api.copy202601.com、api.copy202602.com、api.mangacopy.com、
api.copymanga.fun、api.copy3000.com),所以「挨个试」这条路是通的。
